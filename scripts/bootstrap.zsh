    ...
    while (( # > 0 )); do
        case $1 in
            ...
            all)
                ...
                BOOTSTRAP_FILES=( $~bootstrap_files_pat:t )
                shift
                ;;
            *)
                local frealpath=${1:a}
...
install_with_aur()
{
    ...
    if (( skip_confirm )) || \
        ...
        git clone ${(@)extra_args} $aur_url ${destination:-$HOME/Software/$package_name} && \
            ...
            makepkg -si && rehash || return_code=1
        cd $original_wd
...
install_package()
{
    ...
    if (( #deps )); then
        if [[ $PKG_MANAGER == brew ]]; then
            ...
            sudo pacman -S --needed ${(@)deps} || return 1
        else
    ...
    if (( use_aur )); then
        ...
        if (( use_custom_args )); then
            ...
            (( # > 1 )) && yay_package=$2
            install_arguments=( yay ${(@)extra_args} -S $yay_package )
        elif (( use_git )); then
            (( ! $+commands[git] )) && \
                print -Pn "\n%F{red}%BError:%f%b You cannot install package ${package_name} without git." && \
                return 1
            ...
            (( # > 1 )) && pacman_package=$2
            (( # > 2 )) && pacman_package=$3
            install_arguments+=( sudo pacman -S ${(@)extra_args} $pacman_package )
        else
...
check_packages()
{
    ...
    if (( ! #p10k_paths )); then
        if ! install_package p10k powerlevel10k zsh-theme-powerlevel10k; then
    ...
    if [[ "$(uname -s)" == Linux ]]; then
        ...
        if (( #synhl_paths == 0 )); then
            install_package -y zsh-syntax-highlighting zsh-syntax-highlighting-git
            messages+=1
        fi
    elif [[ "$(uname -s)" == Darwin ]]; then
        if (( #synhl_paths == 0 )); then
...
main()
{
    ...
    if [[ "$(uname -s)" == Darwin ]]; then
        ...
        prepare_package_manager || { print -P "%F{red}%BError%f%b: Homebrew is necessary to perform bootstrapping on macOS." && early_exit 1 }
        local libexec_path=/opt/homebrew/opt/coreutils/libexec/gnubin
