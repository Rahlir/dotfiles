...
vim.g.python3_host_prog = '/Users/rahlir/.pyenv/versions/neovim/bin/python'
-- }}}
-- Autocommands: {{{
...
lightline.component_expand.diag_hint = 'rahlir#hint'
...
-- }}}
-- Lualine: {{{
-- local rahlir_gruvbox = require('lualine.themes.gruvbox')
-- 
-- rahlir_gruvbox.normal.z = {
--   bg = vim.fn.GetGruvColor('GruvboxYellow')[1],
--   fg = rahlir_gruvbox.normal.a.fg
-- }
-- 
-- require('lualine').setup{
--   options = {
--     component_separators = vim.g.separator_bubbles.subseparator,
--     section_separators = vim.g.separator_bubbles.separator,
--     theme = rahlir_gruvbox
--   },
--   sections = {
--     lualine_a = {'mode'},
--     lualine_b = {'filename', 'buffers', {'branch', icon=''}},
--     lualine_c = {
--       {'diff', source = diff_source, symbols = {
--         added = vim.g.gitgutter_sign_added .. ' ',
--         modified = vim.g.gitgutter_sign_modified .. ' ',
--         removed = vim.g.gitgutter_sign_removed .. ' '}
--       }},
...
vim.keymap.set('n', '<leader>gr', function() builtin.lsp_references(themes.get_cursor({
    show_line=false, layout_config={width=0.5}
})) end)
vim.keymap.set('n', '<leader>gd', function() builtin.lsp_definitions(themes.get_cursor({
    show_line=false, layout_config={width=0.5}
})) end)
vim.keymap.set('n', '<leader>gi', function() builtin.lsp_implementations(themes.get_cursor({
    show_line=false, layout_config={width=0.5}
})) end)
vim.keymap.set('n', '<leader>gci', function() builtin.lsp_incoming_calls(themes.get_cursor({
    show_line=false, layout_config={width=0.5}
})) end)
vim.keymap.set('n', '<leader>gco', function() builtin.lsp_outgoing_calls(themes.get_cursor({
    show_line=false, layout_config={width=0.5}
})) end)
